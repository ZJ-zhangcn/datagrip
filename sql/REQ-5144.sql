...
--退保
select 19625.56 + round(round((power(1 + 0.03, 1 / 12) - 1) * 12 / 365, 8)/*将年利率0.03转化为日利率*/*
                       (date'2026-12-25' - date'2026-12-01' + 1) * 19625.56, 2)/*账户价值*/+
       round(1.93 / 31 * 6, 2) 基本保额应退金额
from dual;--19665.71

select 19159.19 + round(round((power(1 + 0.03, 1 / 12) - 1) * 12 / 365, 8)/*将年利率0.03转化为日利率*/*
                       (date'2026-12-25' - date'2026-12-20' + 1) * 19159.19, 2)/*账户价值*/+
       round(0.81 / 11 * 6, 2) 续期应退金额
from dual;--19168.95

select 19665.71+19168.95 from dual;--38834.66
