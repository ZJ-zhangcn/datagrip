...
select /*(select codename from ldcode a where codetype='appflag' and code=a.appflag) */appflag,(select codename from ldcode where codetype='bonusgetmode' and code=a.bonusgetmode) 红利领取方式,(select codename from ldcode where codetype='getlocation' and code=a.getform) 生存金领取方式,contno,prtno,riskcode,signdate,cvalidate,enddate,paytodate,payenddate,payintv,amnt,prem,payendyear,insuyear,a.*,rowid from lcpol a
...
--个险保全表
select edorno 批单号,edortype,edorstate,EdorValiDate 保全生效日期,getmoney,getinterest,a.*,rowid from lpedoritem a where contno='2024041800000116' order by EdorValiDate desc;
...
--退票批处理
select a.*,rowid from ldtask a where taskdescribe like '%退票%' 
