-- ID: REQ-5333
-- 标题: 浦银人寿沪申福富民版终身寿险（分红型）——核心系统需求

--犹豫期
select HESITATEEND
from LMEdorWT
where riskcode = '1116005';

--还款利息
select round(2657.35 * 0.03 / 365 * (date '2026-06-10' - date '2026-05-30' - 1), 2) 还款利息
from dual;
...
where a.riskcode = b.riskcode
  ...
  and a.BONUSGRADE = 'H'
  and a.riskcode = '1116005'
  and date'2027-05-14' between c.CVALIDATE and c.ENDDATE
  and a.POLYEAR = '1'
  and b.contno = '2026051300001586'
...
select round(a.prem / 1000 * b.endcv *
             power(1 + 0.0375, (((date '2026-12-09' - date '2026-05-14') - 365) / 365)), 2) cash,
...
where a.insuredsex = b.gender
  and a.insuredappage = b.age
  and a.payendyear = b.pt
  and a.contno = '2026051300001586'
  and b.dt = 2;

--t>PPP，未欠缴保费
select round((a.prem / 1000 * b.endcv * (date '2048-05-15' - date '2048-05-14') / 365) +
             (a.prem / 1000 * b.begcv *
              (365 - (date '2048-05-15' - date '2048-05-14')) / 365), 2) cash,
       round(a.prem * 1.2, 2)                                            比例,
       round(a.amnt * power((1 + 0.0175), (select dt - 1 from dual)), 2) 有效保险金额
from lcpol a,
     cv_1116005 b
...
--减额交清后保额
select round(6000 / 1000 * a.RPU) 减额交清后保额
from cv_1116005 a,
     lcpol b
where a.AGE = b.INSUREDAPPAGE
  and a.GENDER = b.INSUREDSEX
  and a.PT = b.PAYENDYEAR
  and b.contno = '2026051300001216'
  and a.dt = '2';

--减额交清后的现金价值
select round(7349.22 / 1000 *
             (a.CV_RPUAFACTOREND * (date '2028-05-20' - date '2028-05-14') / 365 +
              a.CV_RPUAFACTORBEG * (365 - (date '2028-05-20' - date '2028-05-14')) / 365), 2) 减额交清后保额
from cv_1116005 a,
     lcpol b
where a.AGE = b.INSUREDAPPAGE
  and a.GENDER = b.INSUREDSEX
...
--减额交清后保费
select round(7349.22 * 1000 / a.RATE, 2) 减额交清后保费
from rt_1116005 a,
     lcpol b
where a.AGE = b.INSUREDAPPAGE
  and a.PT = b.PAYENDYEAR
...
where a.riskcode = b.RISKCODE
  ...
  and to_date('2027-05-15', 'yyyy-mm-dd') between b.startdate and b.enddate
  and to_date('2027-05-15', 'yyyy-mm-dd') between c.cvalidate and c.enddate
  and b.bonusgrade = 'H'
  and b.annuitygetage = '0'
  and e.DUTYCODE like '%1001'/*dt='1'时注释掉*/
  and d.dt = '3'
  and a.contno = '2026051300001126'
order by d.dt;
...
from LJABonusGet a
where contno = '2026040700001076'
order by makedate, maketime;

--退保
--红利宣告交清基本保险金额应退金额
select round((/*152.02 + */a.amnt) / 1000 * cv2 * (date'2029-05-20' - date'2029-05-14') / 365 +
             (/*152.02 + */a.amnt) / 1000 * cv1 * (1 - (date'2029-05-20' - date'2029-05-14') / 365), 2) cash
from lcduty a,
     PUA_1116005 b,
     lcpol c
...
--累积交清基本保险金额减保限额
select round(a.AMNT / 1000 * cv2 * (date'2029-05-20' - date'2029-05-14') / 365 +
             a.AMNT / 1000 * cv1 * (1 - (date'2029-05-20' - date'2029-05-14') / 365), 2) cash
from lcduty a,
     PUA_1116005 b,
     lcpol c
where a.contno = c.contno
...
select round((1 - 100 / 282.22) * 297.58, 2) 减保后保额
from dual;

--基本保额减保限额
select least(round(a.amnt * 0.2 / a.amnt * ((a.prem / 1000 * b.endcv * (date '2031-05-20' - date '2031-05-14') / 366) +
                                            (a.prem / 1000 * b.begcv *
                                             (366 - (date '2031-05-20' - date '2031-05-14')) / 366)) - 0, 2),
             round(((a.prem / 1000 * b.endcv * (date '2031-05-20' - date '2031-05-14') / 366) +
                    (a.prem / 1000 * b.begcv *
                     (366 - (date '2031-05-20' - date '2031-05-14')) / 366)), 2)) 减保限额
from lcpol a,
     cv_1116005 b,
     lcduty c
where a.insuredsex = b.gender
  ...
  and c.dutycode = 'ID0547'
  and a.contno = '2026051300001086'
  and b.dt = 6;

--减保后保额
select round((1 - 1000 / ((a.prem / 1000 * b.endcv * (date '2031-05-20' - date '2031-05-14') / 365) +
                          (a.prem / 1000 * b.begcv *
                           (365 - (date '2031-05-20' - date '2031-05-14')) / 365))) * 9270, 2) 减保后保额
from lcpol a,
     cv_1116005 b,
     lcduty c
where a.insuredsex = b.gender
...
--减保后保费
select round(8291.07 * 1000 / rate, 2) 减保后保费
from lcpol a,
     rt_1116005 b
where a.insuredsex = b.gender
  and a.insuredappage = b.age
