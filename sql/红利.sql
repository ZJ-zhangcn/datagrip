...
SELECT round(0.03 / 730, 8) * 730 * 149.04 +
  ...
  from dual
--相关表结构
select * from lisdata.ldcode where codetype='bonusgetmode';---红利领取方式定义
...
select * from lmriskapp where bonusflag='1' and risktype3='2';--支持分红的产品(1  传统险2  分红3  投连4  万能5  其他)I--个人险、Y--银代险
select * from Lmriskbonus where riskcode='1103004';--1103004
select * from Lobonusrate; ----红利利率定义表(上海人寿)
select * from BonusRate t where t.riskcode='1103007' for update --年度红利分配比例（K）
select * from lobonusfactor t where riskcode = '1103007'
 and t.insuredsex='0' and t.insuyear='6' and t.payendyear='3' and t.insuredappage='24' for update ---红利因子
 select * from lobonusfactor t where riskcode = '1103002'
 and t.insuredsex='0' and t.insuyear='6' and t.payendyear='3' and t.insuredappage='27' 
 for update---红利因子(测试手动配置)
...
select * from Ldbankrate;---累计生息利率
select * from lcpol where bonusgetmode='4';---红利领取方式
select * from Bonus_ENGP  for update---分红费率（建信）
...
---红利的计算	红利=本金+利息
	...
	红利利率表可以查询利率DivRate：select DivRate from LOBonusPolRate where FisCalYear = '2016' and riskcode='BENE12'  
...
select * from LJABonusGet where otherno='2023112900000586';---红利给付实付（资金返盘后置上回盘日期confdate）
select * from LDEdorUser d where UserCode = '001' and UserType = '1'---保全员信息
select * from lcinsureacc where acctype = '004'，------004代表是红利账户
select * from lcinsureacctrace at where at.contno='2023112900000586' ; ---保险帐户表记价履历表
select * from ldcode where codetype = 'companysumbonus';----分红业务红利分配额度
select * from ldcode where codetype = 'appntsumbonus';----分配给投保人的红利总额

select t.FisCalYear,t.divrate,t.publicdate,t.* from LOBonusPolRate t where riskcode='000000';---累计生息时的红利利率表
select * from lcinsureacc where acctype = '004'，------004代表是红利账户
...
如果要回滚红利数据，删除这两个表即可
select * from LJABonusGet where otherno='2024071900000256' for update;
select * from lobonuspol where contno='2024071900000256' for update
